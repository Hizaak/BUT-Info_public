...
int main (int argc, char **argv){
    ...
    Fenetre f;
    Souris s;
    f.apparait("TP4", 600,400,0,0,200,200,200);
    s.associerA(f);

    Carre c1;
    Carre c2(40,40,10, Couleur(255,0,0));
    Carre c3(40,40,20);
    Carre c4 (c3);

    CaseACocher cac1 (40,40,60,Couleur(255,0,0), 'h');
    CaseACocher cac2 (c3,'C');
    CaseACocher cac3(cac2);
    CaseACocher cac4;

    // test pour affichage quand héritage
    cac1.afficher(f);
    cac1.effacer(f);
	...
	c1.couleur().bleu()== 255)
        ...
        f.ecrit(10,40,"Constructeur par defaut de la classe Carre : OK");
    else
        f.ecrit(10,40,"Constructeur par defaut de la classe Carre : PAS OK");

    if (c2.cote() == 10 &&
	c2.coordX() == 40 &&
	c2.coordY() == 40 &&
	c2.couleur().rouge()== 255 &&
	c2.couleur().vert()== 0 &&
	c2.couleur().bleu()== 0 &&
	c3.couleur().rouge()== 0 &&
	c3.couleur().vert()== 0 &&
	c3.couleur().bleu()== 255)
        ...
        f.ecrit(10,60,"Constructeur (int, int, int, Couleur) de la classe Carre : OK");
    else
        f.ecrit(10,60,"Constructeur (int, int, int, Couleur) de la classe Carre : PAS OK");
	...
	cac4.cadre().couleur().bleu()== 255 &&
	cac4.etat()=='N')
        f.ecrit(10,80,"Constructeur par defaut de la classe CaseACocher : OK");
    else
        f.ecrit(10,80,"Constructeur par defaut de la classe CaseACocher : PAS OK");

    if (cac1.cadre().cote() == 60 &&
	cac1.cadre().coordX() == 40 &&
	cac1.cadre().coordY() == 40 &&
	cac1.cadre().couleur().rouge()== 255 &&
